...
public class HashMap<K, V> extends AbstractMap<K, V>
    ...
    static final int hash(Object key) {
        int h;
        // 1： 为什么要使用 ^(异或) 运算符 ?
        // 主要是因为 在 get(key) 方法中 是通过  tab[(n - 1) & hash]) 中来定位下标的；在这如果不使用 ^ (h >>>16) 运算符, 假设 n = 16 (初始长度)，A对象hash值为： 10000000，B对象的hash值为11000000
        // 那么 就是 15(00001111) 和 hash值来取数，15分别于A,B 进行&运算 得到的结果都为 0 ；这样就会导致key值重复;
        // 如果要是加上 ^(h >>> 16) A 右移16位位 8个0，然后与 A的hashCode进行 ^ 进行运算 得到 10000000 还是原来的值，B同样也还是得到原来的值为 11000000 ；这样就不重复了
        // 总结，使用该运算符，就是为了避免hash值不重复。因为在定位下标的时候要用
     ...
     *
     * @param hash         hash for key 如果 key 是 String 类型的，hash  就是 通过 String 的 hashcode() 得到的
     * @param key          the key
    ...
    final V putVal(int hash, K key, V value, boolean onlyIfAbsent,
        ...
        // (n - 1) & hash ; 假设 n = 16 那么就是 15(1111) & hash值 就会根据hash值的不同得到 <= 15的下标;
        // 为什么使用(n-1) & hash 取代了 hash % 16 ?
